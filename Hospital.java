...
public class Hospital {
	...
	if(address!=null){
	   for(int i=0;i<patients.length;i++){
	     if(patients[i].getAddress().getAddressName().equals(address)){
...
public PatientDTO getPatientByStateName(String stateName) {
	PatientDTO patientDTO=null;
	System.out.println("inside getStateByStateName()");
	if(stateName!=null) {
		for(int i=0;i<patients.length;i++) {
			if(patients[i].getAddress().getCountry().getState().getStateName().equals(stateName)) {
			
	        System.out.println("Patient found by Name:"+stateName);
			patientDTO=patients[i];
		}
	

else{
	System.out.println("No Patient found by Name:"+stateName);
}
		}
	}
return patientDTO;
 ...
 public PatientDTO getPatientByStreetNo(int streetNo) {
	PatientDTO patientDTO=null;
	System.out.println("inside getStreetByStreetNo()");
	if(streetNo!=0) {
		for(int i=0;i<patients.length;i++) {
...
if(patients[i].getAddress().getCountry().getState().getDistrict().getArea().getStreet().getStreetNo()==(streetNo)) {   
			...
			System.out.println("Patient found by No:"+streetNo);
			patientDTO=patients[i];
		}
	

   else{
	System.out.println("No Patient found by No:"+streetNo);
}
		}
	}
return patientDTO;
...
System.out.println("Patient found by StreetName:"+streetName);
	...
	System.out.println("No Patient found by No:"+streetName);
}
		}
	}
return patientDTO;
    ...
    PatientDTO patientDTO=null;
	System.out.println("inside getAreaByAreaName()");
	if(areaName!=null) {
		for(int i=0;i<patients.length;i++) {
			PatientDTO patient=patients[i];
...
if(patients[i].getAddress().getCountry().getState().getDistrict().getArea().getAreaName().equals(areaName)){
			System.out.println("Patient found by Name:"+areaName);
			patientDTO=patients[i];
		}
	

else{
	System.out.println("No Patient found by Name:"+areaName);
}
		}
	}
return patientDTO;
